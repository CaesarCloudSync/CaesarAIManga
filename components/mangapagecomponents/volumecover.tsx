...
export default function VolumeCover({mangaid,cover_art,volumeno,title,cover_id,type,chapterid,currentpage,setRecentManga,chaptertitle,offline}:any){
    ...
    console.log(offline === true ? FileSystem.documentDirectory + cover_art:`https://uploads.mangadex.org/covers/${mangaid}/${cover_art}` )
    return(
        <View style={{flex:1,justifyContent:"center",alignItems:"center",backgroundColor:volumecolor}}>
            <TouchableOpacity onLongPress={() =>{removefromrecentreading()}} onPress={() =>{navtochapters()}}>
                <Image style={{width:175,height:250}} alt="hello" source={{uri:offline === true ? FileSystem.documentDirectory + cover_art:`https://uploads.mangadex.org/covers/${mangaid}/${cover_art}` }}></Image>
